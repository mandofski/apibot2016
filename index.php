<?php
ob_start();
define('API_KEY','249031900:AAGN-SuXJXg3R5dFBBtxBHGJywm5cEBIp2Y');
$the_admin_id = 156292820;
                ...
                'results' => json_encode([[
                    ...
                    [
                        ...
                        'input_message_content'=>[
                            'message_text'=>'@inlinerbotlt_bot getlist-'.$theListId
                        ]
                    ]])
            ]));
        }else{
...
<pre>@inlinerbotlt_bot d6cdbea45b238632bdd6d11dcf7fe98f </pre>
...
قبل از هر کاری اول باید بدانید این via inlinerbotlt_bot چیست !
...
<code>@inlinerbotlt_bot getlist-XXXXX </code>
...
👈  حالا برای من کد inline تان را بفرستید : ( فعلا فقط لیست ها قبول هستند )".$cancel_Text,
                ...
                case '2':{
                    ...
                    if(str_replace('@inlinerbotlt_bot getlist-','',$code) != $code){
                        $code_list = trim(str_replace('@inlinerbotlt_bot getlist-','',$code));
...
به ربات ما سر بزنید
https://telegram.me/inlinerbotlt_bot
                            "
                        ]);
                    }else{
                        $newList = [];
...
اگر بد توضیح دادم روی /help کلیک کن راهنمایی مفصل داره !'
                            ...
                            $theCommand = '@inlinerbotlt_bot getlist-'.ListTab::getCurrentMakeID($update->message->from->id);
                            makeHTTPRequest('sendMessage',[
                                'chat_id'=>$update->message->from->id,
                                'text'=>$theCommand,
                                'reply_markup'=>json_encode([
...
اگر بد توضیح دادم روی /help کلیک کن راهنمایی مفصل داره !'
                    ...
                    $theCommand = '@inlinerbotlt_bot getbanner-'.getCurrentMakeID($update->message->from->id);
                    makeHTTPRequest('sendMessage',[
                        'chat_id'=>$update->message->from->id,
                        'text'=>$theCommand,
                        'reply_markup'=>json_encode([
...
//                    ]);
//                    makeHTTPRequest('sendMessage',[
//                        'text'=>'@inlinerbotlt_bot getbanner-'.getCurrentMakeID($userID),
//                        'chat_id'=>$userID
//                    ]);
//                }break;
